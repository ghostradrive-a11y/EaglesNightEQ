...
private:
    ...
    using MonoChain = juce::dsp::ProcessorChain<
        Filter,  // Low shelf (warmth)
        Filter,  // Low-mid cut (clarity)
        Filter,  // Notch at 2.3kHz (user adjustable)
        Filter,  // Presence / upper-mid smooth
        Filter   // High shelf (air)
    >;
    ...
    void updateFilters();
    void updateCoefficients(const Filter::CoefficientsPtr& coefficients, int position);
